...
class FaceRecognitionProcessor:
    ...
    async def upload_face_to_s3_async(self, face_image, key):
        ...
        async with session.client(
            ...
            await s3.put_object(Bucket="located-faces", Key=key, Body=face_image)
    ...
    def upload_face_to_s3(self, face_image, key):
        ...
        return asyncio.run_coroutine_threadsafe(
            self.upload_face_to_s3_async(face_image, key), self.loop
        )
    ...
    def process_video_stream(self, monitor: CCTVCamera):
        ...
        while not self.stop_event.is_set() or not monitor.is_deleted:
            try:
                ...
                if not ret:
                    logging.warning(
                        f"Failed to capture frame from {monitor.monitor_id}"
                    )
                ...
                if (
                    video_capture.get(cv2.CAP_PROP_POS_FRAMES) % process_every_nth_frame
                    == 0
                ):
                    ...
                    for face_encoding, face_location in zip(
                        ...
                        for user_id, dataset in monitor.users.items():
                            ...
                            if matches[best_match_index]:
                                logging.info(
                                    f"{monitor.monitor_id} Found: {user_id} with distance: {face_distances[best_match_index]}"
                                )
                                face_matched.append(user_id)

                                if (
                                    user_id in monitor.face_tracking.keys()
                                    and not monitor.face_tracking.get(user_id)
                                ):
                                    monitor.face_tracking[user_id] = (
                                        RegisteredUserCache(
                                            ...
                                            image_url=self.save_user_face_to_s3(
                                                user_id, frame, face_location
                                            ),
                                        )
                                    )

                    self.check_face_detection_cache(monitor, face_matched)
                    reconnect_stream = self.check_live_tracking_cache(
                        monitor, face_matched
                    )
                    if reconnect_stream:
                        logging.warning(
                            f"{monitor.monitor_id} reconnection to {monitor.monitor_id}"
                        )
                        video_capture = cv2.VideoCapture(monitor.monitor_stream_url)
        ...
        with self.lock:
            ...
            for user_id in missing_users:
                ...
                if payload and not payload.disappeared_at:
                    ...
                    payload
                    and payload.disappeared_at + timedelta(seconds=30) < datetime.now()
                ):
                    # send update history
    ...
    def check_live_tracking_cache(self, monitor: CCTVCamera, matched_users: List[int]):
        with self.lock:
            ...
            for user_id in matched_users:
                ...
                if not payload:
                    monitor.live_tracking[user_id] = {"appeared_at": datetime.now()}
            ...
            for user_id in missing_users:
                ...
                if (
                    disappeared_at
                    and disappeared_at + timedelta(seconds=30) < datetime.now()
                ):
                    ...
                    not disappeared_at and "appeared_at" in missing_user_payload.keys()
                ):
                    missing_user_payload["disappeared_at"] = datetime.now()
                logging.info(
                    f"{monitor.monitor_id} missing user payload: {missing_user_payload}"
                )

            user_under_tracking = [
                user_id for user_id, payload in monitor.live_tracking.items() if payload
            ]
            logging.info(
                f"{monitor.monitor_id} user under tracking: {user_under_tracking}"
            )

            # Do we need enable recording?
        if monitor.state == MonitorStateEnum.start and user_under_tracking:
            logging.info(
                f"{monitor.monitor_id} enable monitor recording {monitor.monitor_id}"
            )
            ...
            logging.info(
                f"{monitor.monitor_id} no user under tracking {monitor.monitor_id}. stop record"
            )
            with self.lock:
    ...
    def get_monitor(self, monitor_id: str) -> CCTVCamera:
        for monitor in self.monitors:
            if monitor.monitor_id == monitor_id:
                return monitor
