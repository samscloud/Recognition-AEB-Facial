...
class Config(BaseSettings):
    ...
    BACKEND_API_URL: str = Field(
        "https://api.samscloud.io/api/v2/organizations", env="BACKEND_API_URL"
    )
    ...
    AWS_STORAGE_BUCKET_NAME: str = Field(
        "user-faces-dataset", env="STORAGE_BUCKET_NAME"
    )
    ...
    USER_NOTIFICATION_DELAY_SECONDS: int = Field(
        30, env="USER_NOTIFICATION_DELAY_SECONDS"
    )
