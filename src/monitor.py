    ...
    def __generate_hls_stream(
        ...
        try:
            while not self.stop_event.is_set():
                ...
                # if cap.get(cv2.CAP_PROP_POS_FRAMES) % process_every_nth_frame == 0:
                #     face_matched, face_unmatched = self.face_detection_model.process(frame)
                #     print(f"Face matched: {len(face_matched)} | Face unmatched: {len(face_unmatched)}")
