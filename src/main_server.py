...
class MainServer:
    ...
    async def get_monitor_by_id(
        self, monitor_id: str
    ) -> Optional[OrganizationCameraSchema]:
        url = f"https://api.samscloud.io/api/v2/organizations/{self.organization_slug}/cctv/monitors/{monitor_id}/"
        response = await self.__send_get_request(url)
        if response is None:
            raise MainBEException

        return OrganizationCameraSchema(**response)
