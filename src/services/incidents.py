...
class IncidentService:
    ...
    async def create_new_alert(self, monitor_id: str, camera_id: int, message: str):
        ...
        try:
            await self.shinobi_client.start_monitor_recording(monitor_id)
    ...
    async def update_ongoing_alert(self, monitor_id: str, camera_id: int):
        ...
        try:
            ...
            print(url)
            response = await self._request(url, payload.model_dump(mode="json"))
            print(response)
            ...
            wss = self.ws_connections.get(monitor_id)
            if wss:
                print("send websocket for new camera added")
                await wss.send_message(message)
        except Exception as e:
            print(e)
    ...
    async def end_ongoing_incident(self, monitor_id: str, message: str):
        ...
        try:
            await self.shinobi_client.stop_monitor_recording(monitor_id)
        except Exception as e:
            print(f"monitor recording start: {e}")
        ...
        msg = json.dumps(
            {
                ...
                "payload": {
                    ...
                    "camera_status_changed": (
                        "watching"
                        if self.shinobi_client.recorded_monitor_register.get(
                            monitor_id, 0
                        )
                        == 0
                        else "record"
        ...
        if wss:
            print("send websocket for incident end")
            await wss.send_message(msg)
