...
from src.schema import NewCameraRequestSchema, VideoSchema, MonitorSchema
...
class Shinobi:
    ...
    def register_monitor(self, monitor: OrganizationCameraSchema):
    ...
    def video(self) -> List[VideoSchema]:
        ...
        for item in response.json().get("videos", []):
            start = datetime.strptime(item.get("time"), "%Y-%m-%dT%H:%M:%SZ")
            end = datetime.strptime(item.get("end"), "%Y-%m-%dT%H:%M:%SZ")
            ...
            if (end - start).seconds > 2:
                videos.append(
                    VideoSchema(
                        ...
                        video_url=item.get("href", "").replace(
                            "http://127.0.0.1:8080", "https://many-pets-judge.loca.lt"
                        ),
                    )
                )
        return videos
